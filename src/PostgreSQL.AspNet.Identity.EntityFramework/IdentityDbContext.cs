	...
	public class IdentityDbContext<TUser> :
		...
		internal static bool IsIdentityV1Schema(DbContext db) {
			...
			if (db.Database.Exists()) {
				using (var tempConnection = new NpgsqlConnection(originalConnection.ConnectionString)) {
					tempConnection.Open();
					return
		...
		internal static bool VerifyColumns(NpgsqlConnection conn, string table, params string[] columns) {
			var tableColumns = new List<string>();
			using (
				var command =
					new NpgsqlCommand("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS where TABLE_NAME=@Table", conn)) {
