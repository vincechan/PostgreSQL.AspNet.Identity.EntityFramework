...
namespace PostgreSQL.AspNet.Identity.EntityFramework {
	...
	public class UserStore<TUser, TRole, TKey, TUserLogin, TUserRole, TUserClaim> :
		...
		/// <summary>
		/// Returns the current number of failed access attempts. This number usually will be reset
		/// whenever the password is verified or the account is locked out.
		/// </summary>
		/// <param name="user"></param>
		/// <returns></returns>
		...
		/// <summary>
		/// Returns the DateTimeOffset that represents the end of a user's lockout, any time in the
		/// past should be considered not locked out.
		/// </summary>
		/// <param name="user"></param>
		/// <returns></returns>
		...
		private static class FindByIdFilterParser {
			...
			internal static bool TryMatchAndGetId(Expression<Func<TUser, bool>> filter, out TKey id) {
				...
				if (callExpression.Arguments[0].NodeType == ExpressionType.Convert) {
					...
					// convert node should have an member access access node This is for cases when
					// primary key is a value type
